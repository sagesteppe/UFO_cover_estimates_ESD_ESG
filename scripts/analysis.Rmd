...
dirtyNet <- ESD2ESG_wide %>% 
  ...
  # now join the correct ESG to the reference ESD's
```
...
cleanNET3 <- ESD2ESG_wide %>% 
  ...
  distinct() %>% 
  slice(rep(1:n(), each = NO_ESD)) # now have to remove these from the next
...
Extract from the the three layers, detect most probable. 
```{r}
