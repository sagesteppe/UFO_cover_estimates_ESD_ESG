...
finalNET <- ESD2ESG_wide %>% 
  ...
  mutate(ESG = str_replace(ESG, '-_|_-_', '-'), 
         ...
         ESG = str_replace(ESG, 'SAW-Loamy_Uplands', 'SAW-Sandy_and_Loamy_Uplands')
         ) %>% 
  arrange(ESG)
...
tr <- read.csv(file.path(praw, f[grep('FractionalCover', f)] ))
...
benchcover <- ggplot(ob, aes(MEAN_X, MEAN_Y, color = MODEL), size = 3, alpha = 0.5) +
  geom_jitter() +
...
```{r Plot final benchmark cover against each other, fig.height = 14, fig.width = 10}
benchcover
```
