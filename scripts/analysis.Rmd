...
rm( theme_boxplot,
   theme_prop_bar, esg, esg_correct, test_results)
```
...
rm(ob, mod, EsdEsg_network, modules)
```
...
cleanNet <- rbind(cleanNET1, cleanNET2, cleanNET3)
...
rm(ob, EsdEsg_network, mod)
```


While the final output classification gridded surface has a classification accuracy of 67% the next two output layers, have slightly different predictions and 86% of the time will contain the correct ESG. Here we will extract all three layers to a site, sum all of the results across them and search for the most probable result.
...
extract(c_zones, ecosites, method = 'simple') %>% 
  group_by(ID) %>% summarise(ClimZone = mode(ClimZone)) 
...
data.frame(levels(sgu1))

rm(sgu1_ex, sgu1, sgu2, sgu3, praw, f, ecosites)
```
...
The method of extracting the ESG from th gridded surface to plot had virtually no influence on the accuracy of the ESG. Regardless the method of buffering the point to the real size of the AIM plot was used as it slightly outperformed plot centers on the `r nrow(known)` known sites,  `r p2p[(p2p$test == 'point'),]$Prop` to `r p2p[(p2p$test == 'polygon'),]$Prop`.
...
```{r}
rm(known, prop_D_map2_G, number_correct_esg, number_incorrect, p2p)
```
