...
benchcover <- ggplot(ob, aes(MEAN_X, MEAN_Y, color = MODEL)) +
  geom_jitter(size = 4.5, alpha = 0.5) +
  ...
  theme(aspect.ratio=1,
        ...
        legend.title = element_text(size=14, hjust = 0.5), #change legend title font size
        legend.text = element_text(size=12),
        panel.grid.minor=element_blank()
        ) +
  scale_color_manual('Reference:',  values = c('ESD' =  '#10A19D', 'ESG' = '#FF7000')) +
  labs(title = 'Comparison of the mean of Benchmarks between ESD & ESG', 
       x = 'mean % fractional cover (any hit)', y = 'mean % fractional cover (any hit)')
...
distributions <- ggplot(ob1, aes(x=MEAN_X, y = dum)) + 
  ...
  scale_color_manual('Reference:',  values = c('ESD' =  '#10A19D', 'ESG' = '#FF7000')) +
    labs(title = NULL, 
       x = 'mean % fractional cover (any hit)', y = NULL)

# benchcover
# distributions

rm(lt, lookuptablegrps, lookuptable)
```
...
par(mfrow=c(2,1), mar=c(0.0, 0.0, 0.2, 0.2))
...
```{r Plot final benchmark cover against each other, fig.height = 14, fig.width=10, fig.cap="Fractional cover, note the provisional ESGS for Outcrops and Riparian, groups 7 & 8, have been removed."}
benchcover

# `r  which.max(ob[ob$COVER_TYPE == 'TREE' & ob$MODEL == 'ESG',]$MEAN)`
```

`r obG <- ob[ob$MODEL == 'ESG',] `
...
Greater divergences are observed between the estimates of cover for Grass and Shrub. The cover of grasses in XX ESG's, XX, are conceptually identical, however in the other XX ESG's the cover of grass notably exceeds that noted in the ESD's (*Fig 5 & 6, Column 2*). The reasons for this are perhaps ... The relationships between the shrub cover values are more nuanced, XX ESG's have conceptually identical cover values, however for the remaining ESG's the cover of Shrubs is less than noted in the ESD's (*Fig 5 & 6, Column 3*). That the cover of shrubs would be less than expected is unanticipated, given the noted movement of much of the vegetation in the arid portions of Western North America moving towards a 'shrub state', a condition whereby the cover of shrubs has increased relative to uncommon parcels of undisturbed land (CITE). 


```{r Box Plot of Mean Covers, fig.cap="Fractional cover across all ESG's and the ESD's with quantitative Benchmarks within them."}
distributions
```


```{r determine relationship between mean values for text write up, eval = F}

ob_covers <- ob %>% 
  group_by(ESGID) %>%
  mutate(MEAN_MAX = MEAN + 5, 
         MEAN_MIN  = MEAN - 5) %>% 
  mutate(BENCH_RELATION = case_when(
    MODEL == 'ESG' & MEAN + 5 & MEAN - 5 
    
  ))

ob[ob$MODEL == 'ESG' & ob$COVER_TYPE == 'GRASS',]
ob[ob$MODEL == 'ESD' & ob$COVER_TYPE == 'GRASS', ]
```

The distributions of covers for each lifeform regardless of ESG are reported in the bottom of (figure 5, Pane B.). 
