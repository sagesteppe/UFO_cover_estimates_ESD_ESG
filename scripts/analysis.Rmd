...
ggplot(esg_correct, aes(y = Response_V, x = ESG, fill= Response)) +
    ...
    scale_fill_manual('Identification:', 
                      values = c('Incorrect' = 'darkred', 'Correct' = 'darkslategray3')) +
  scale_x_discrete(guide = guide_axis(n.dodge=10)) +
  labs(title = 'ESGs which were mapped to from Raster', y = 'Number', x = 'Ecological Site') 

# Any ESG in red is an inaccurate result. The actual number of ESG's which should be present
# in this sample are those in light blue. 
...
ggplot(esd_correct, aes(y = Response_V, x = ECO.SITE, fill= Response)) +
  ...
  labs(title = 'Proportion of Plots mapped to ESG', y = 'Number', x = 'Ecological Site') 
...
rm( mode, theme_boxplot, esd_correct,
   theme_prop_bar, esg, esg_correct, ref_lookup, test_results, poly2point)
```
...
`r nrow(known)` AIM points were in the look up values for ESG... XX of these ESDs were included directly in the making of the ESGs . The ESG gridded surface was able to correctly place XX plots to the appropriate ESG. 

When extracting a data set of X AIM plots with classified ESD's which are included in the ESG reference materials the number of `r number_incorrect`, `r number_correct_esg`
