...
The mean fractional cover of vegetation was manually transcribed from Table 3 of Appendix A from Nauman et al. 2022. The spatial data product which the study which was the outcome of the study was accessed from sciencebase.gov catalog on (Dec. 16, 2022). 
...
ref_lookup <- read.csv(file.path(praw, f[grep('Appendix',f)] )) %>% 
  ...
  mutate(
    ...
    Final.Soil.Geomorphic.Unit = to_parsed_case(Final.Soil.Geomorphic.Unit)
    ) %>% 
...
Multiple options for extracting raster values to points were utilized to try and determine the correct ESG. Three parameters (plot buffer, summary statistic, and bilinear interpolation) were altered in two dichotomous ways (Yes/No), and all but one combinations thereof (the combination of a true point geometry and statistical summary methods cannot coexist) were implemented to determine which gave the best results.  

*Bilinear interpolation* would be expected to "smooth" the surface of the raster dataset, and favor the most locally common ESG, whereas *simple interpolation* should be have a higher ability to detect more locally rare ESG's. Unfortunately the less common ESG's are not present in the validated lookup dataset. *Buffering* the center of the plot to match it's real world extent should also detect the more prevalent ESG's at the expense of less locally common ones; theoretical buffering would result in the highest number of mis-classifications, as the soil parameters which existed at plot center where almost the metric exclusively used in matched ESD's. *MODE OF CLASSIFICATION ADDED BACK IN*


```{r Which raster extraction methods gave best results}
...
ab <- test_results %>% 
  filter(test == 'polygon') %>% 
  group_by(ECO.SITE) %>% 
  mutate(PROP_ES_MAPPED = sum(RESULT)) %>% 
  distinct(ECO.SITE, RESULT, .keep_all = T)

# Proportion of ESG which is correctly identified

test_results %>% 
  filter(test == 'polygon') %>% 
  group_by(ESG) %>% 
  mutate(Correctly_Assigned = sum(RESULT), ) %>% 
  distinct(ESG, .keep_all = T)

# Proportion of which ESD is correctly identified
